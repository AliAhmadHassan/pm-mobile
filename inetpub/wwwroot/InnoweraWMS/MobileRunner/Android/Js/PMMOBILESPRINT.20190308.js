...
$(document).ready(function () {
	...
	$("[itag='btnDisplayPontoMedicao']").on("touchend", function (e) {
		...
		$('#divdivPontoMedicao #divPontoMedicaoDadosGerais .panel-heading .panel-title').text("Criar Documento de Medição");
...
function divDisplayOrderDetails_btnEdit() {
	try {
		...
		$('#divDisplayOrderDetails #divOperacoes #btnEdit').show();
...
function divDisplayOrderDetails_btnCancel() {
	try {
		...
		$('#divDisplayOrderDetails #divOperacoes #btnEdit').hide();
	...
	try {
		...
		for (let i = 0; i < txtNPessoalSep.length; i++) {
			...
			if (nPessoal.length > 0) {
				...
				linhas += '	<td><input type="date" id="txtDtInicioExecucao" value="' + txtDtInicioExecucao + '" style="width: 80px"/></td>';
				linhas += '	<td><input type="time" id="txtHrInicioExecucao" value="' + txtHrInicioExecucao + '" style="width: 80px"/></td>';
				linhas += '	<td><input type="date" id="txtDtFimExecucao" value="' + txtDtFimExecucao + '" style="width: 80px"/></td>';
		...
		function ProcessOperationSuccess(jsonRecordWithResult) {
			...
			try {
				...
				if (result.ReturnTableCollection[0][1].length > 0) {
					$("#divPontoMedicao #divUltimoDocMedicao #txtDocumentoMedicao").val(result.ReturnTableCollection[0][1][0][0]);
					$("#divPontoMedicao #divUltimoDocMedicao #txtPontoMedicao").val(result.ReturnTableCollection[0][1][0][1]);
				...
				if (result.SapReturnTable[0].K != "") {
					$("#divPontoMedicao #divPontoMedicaoCriar #txtEquipLocInst").val(result.SapReturnTable[0].K);
					$("#divPontoMedicao #divPontoMedicaoCriar #txtDenominEquipLocInst").val(result.SapReturnTable[0].L);
					$("#divPontoMedicao #txtEquip").val(result.SapReturnTable[0].K);
				...
				$("#divPontoMedicao #txtValorTeorico").val(result.SapReturnTable[0].X);
...
function call_ZPMF_IW47_LISTAR_CONF() {
	try {
		...
		function ProcessOperationSuccess(jsonRecordWithResult) {
			...
			try {
				//alert(JSON.stringify(result.SapReturnTable[0]));
				DataBinding($('#divConfirmacaoColetiva #showListaConfirmacoes table[itable="tblMatchCode"]'), result.ReturnTableCollection[0], false, false);
...
function call_ZPMF_IK12_OBTER_DADOS() {
	try {
		...
		function ProcessOperationSuccess(jsonRecordWithResult) {
			...
			try {
				//alert(JSON.stringify(result));
				$("#divModificarPontoMedicao #divPontoMedicaoDadosGerais #txtPoint").val(result.SapReturnTable[0].B);
				...
				if (result.ReturnTableCollection[0][1].length > 0) {
					$("#divModificarPontoMedicao #divUltimoDocMedicao #txtDocumentoMedicao").val(result.ReturnTableCollection[0][1][0][0]);
					$("#divModificarPontoMedicao #divUltimoDocMedicao #txtPontoMedicao").val(result.ReturnTableCollection[0][1][0][1]);
				...
				if (result.SapReturnTable[0].E != "") {
					$("#divModificarPontoMedicao #divPontoMedicaoDadosGerais #txtEquipLocInst").val(result.SapReturnTable[0].E);
...
function call_IK11_MEASUREMENT_36571_medidor() {
	try {
		...
		var txtPoint = $('#divPontoMedicao #divPontoMedicaoCriar #txtPoint').val();
		var txtLidoPor = $('#divPontoMedicao #divPontoMedicaoCriar #txtLidoPor').val();
		var txtDataMedicao = $('#divPontoMedicao #divPontoMedicaoCriar #txtDataMedicao').val().replace("/", ".").replace("/", ".").replace("/", ".");
		...
		var column = ['E', 'C', 'A', 'B', 'D'];
		var row = [txtPoint, txtLidoPor, txtDataMedicao, txtHoraMedicao, txtValorMedido];
		var temp = [];
		temp.push(column);
		temp.push(row);
...
function call_divIW41_CONFIRM_ORDER() {
	try {
		...
		var txtDtInicioConfirmada = Convert_yyyyMMdd($('#divIW41_CONFIRM_ORDER #txtDtInicioConfirmada').val()).replace('/', '.').replace('/', '.').replace('/', '.');
		var txtHoraInicioConfirmada = $('#divIW41_CONFIRM_ORDER #txtHoraInicioConfirmada').val();
		var txtDtFimConfirmada = Convert_yyyyMMdd($('#divIW41_CONFIRM_ORDER #txtDtFimConfirmada').val()).replace('/', '.').replace('/', '.').replace('/', '.');
...

			{
				...
				result.ReturnTableCollection[6][1].forEach(item => {
					...
					if (tabelaCaracteristicasTipo[3] == "X") {
						...
						CampoEdicao += '<select id="ddlCaracteristica" class="form-control" style="width:300px; min-width:300px; max-width:300px; display:inline-block !important;">'
						...
						CampoEdicao += '</select>';
						CampoEdicao += '<input type="text" class="form-control" style="width:initial; min-width:initial; max-width:initial; display:none !important;" id="txtCaracteristica" value="' + item[1] + '"/> ';
						CampoEdicao += '<input type="checkbox" id="cbxCaracteristica"> Valor Adicional </input>';
					} else {
					...
					linhaCaracteristicas += "   <td style='display:none; width:50%; min-width:50%; max-width:50%;'>" + CampoEdicao + "</td>";
...

			{
				...
				result.ReturnTableCollection[7][1].forEach(item => {
					...
					if (tabelaMatchCodeCaracteristicas.length > 0) {
						...
						CampoEdicao += '</select>';
						CampoEdicao += '<input type="text" class="form-control" style="display:none !important;" id="txtCaracteristica" value="' + item[1] + '" />';
						CampoEdicao += '<input type="checkbox" id="cbxCaracteristica"> Valor Adicional </input>';
					} else {
...
function call_IL02_BAPI_OBJCL_CHANGE() {
	try {
		...
		function ProcessOperationSuccess(jsonRecordWithResult) {
			...
			$('#exibeLocalInstalacaoDadosGerais #TxtGrpPlanej').attr('readonly', 'readonly');
			$('#exibeLocalInstalacaoDadosGerais #TxtCentroCusto').attr('readonly', 'readonly');
			$('#divExibeLocalInstalacao #exibeLocalInstalacaoCaracteristicas #btnEditar').show();
			$('#divExibeLocalInstalacao #exibeLocalInstalacaoCaracteristicas #btnSalvar').hide();
			$('#divExibeLocalInstalacao #exibeLocalInstalacaoCaracteristicas #btnCancelar').hide();
			call_ZPMF_IL03_EXIBIR_LOCAL();
		}
...
function IW22DadosGerais_btnSave() {
	...
	try {
		...
		jsonInformation.PostType = PostType;
		jsonInformation.ProcessType = FileType;
		jsonInformation.FormID = "Form20181205095913019";
		jsonInformation.Action = strAction;
		...
		var arrayHeader = [];
		var header = ['S','Q','O','K','C','G','E','I','T','R','P','L','D','H','F','J','A'];
		var item = [txtEquip,txtLocInst,txtTextoBreveNota,txtParada,txtInicioAvariaData,txtInicioAvariaHora,txtFimAvariaData,txtFimAvariaHora,'X','X','X','X','X','X','X','X',txtNota];
...
function resetConfirmacaoIndividual() {
	...
	$('#divIW41_CONFIRM_ORDER #cxbConfirmacaoParcialFinal').removeAttr("checked");
}
